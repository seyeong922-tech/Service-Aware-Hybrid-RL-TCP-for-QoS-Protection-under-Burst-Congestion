...
NS_LOG_COMPONENT_DEFINE("Topo3DumbbellAsym");
...
constexpr uint32_t SERVICE_TYPE_FTP = 1;
constexpr uint32_t SERVICE_TYPE_VIDEO = 2;

const std::string RESULT_DIR = "scratch/results/dumbbell_asym_rtt/";
...
{
    // Asymmetric dumbbell traffic direction:
    // S1/S2/S3 -> R1 -> R2 -> D1/D2/D3
    // Therefore, the congested output queue is R1->R2, dR1R2.Get(0).
...
int main(int argc, char *argv[])
{
    ...
    // Asymmetric access delays emulate heterogeneous local devices/users.
    // This is not identical to the star-like gateway evaluation topology,
    // but it exposes the model to RTT/access asymmetry during training.
    accessS1.SetDeviceAttribute("DataRate", StringValue("100Mbps"));
    accessS1.SetChannelAttribute("Delay", StringValue("3ms"));
    ...
    accessS3.SetChannelAttribute("Delay", StringValue("5ms"));
    ...
    // Asymmetric dumbbell topology:
    //
    // S1 FTP/background --3ms--\
    // S2 primary video  --1ms--- R1 ===== R2 --- D1/D2/D3
    // S3 burst traffic  --5ms--/
    //
    // R1->R2 is the shared bottleneck.
    //
    // Training purpose:
    // - expose the general model to a shared bottleneck with asymmetric access RTTs
    // - keep S1/S2/S3 semantics identical to parking-lot and dumbbell
    // - do not duplicate the star-like gateway evaluation topology
    NetDeviceContainer dS1R1 = accessS1.Install(S.Get(0), R.Get(0));
    NetDeviceContainer dS2R1 = accessS2.Install(S.Get(1), R.Get(0));
    NetDeviceContainer dS3R1 = accessS3.Install(S.Get(2), R.Get(0));
    NetDeviceContainer dR1R2 = bottleneck.Install(R.Get(0), R.Get(1));
    NetDeviceContainer dR2D1 = serverAccess.Install(R.Get(1), D.Get(0));
    NetDeviceContainer dR2D2 = serverAccess.Install(R.Get(1), D.Get(1));
    NetDeviceContainer dR2D3 = serverAccess.Install(R.Get(1), D.Get(2));
    ...
    address.SetBase("10.6.7.0", "255.255.255.0");
    ...
    NS_LOG_UNCOND("Mode: " << mode
                  << ", Topology: Asymmetric Dumbbell"
                  << ", TCP: TcpCubic"
                  << ", Bottleneck: " << BOTTLENECK_RATE
                  << ", Queue: " << QUEUE_SIZE
                  << ", AccessDelay(S1/S2/S3): 3ms/1ms/5ms"
                  << ", Step: " << STEP_TIME
